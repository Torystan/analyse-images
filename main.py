...
class Main():
    ...
    def __init__(self):
        ...
        self.cap2 = cv2.VideoCapture(os.path.dirname(__file__) + "/video/videoBabord.mp4")
        ...
        self.analyses = {}
        self.analyses["derive"] = AnalyseDerive(1462, 271, 1514, 410)
        self.analyses["safran"] = AnalyseSafran(355, 433, 451, 572, 411, 435, 422, 506)
        self.analyses["mousse"] = AnalyseMousse(570, 380, 715, 637)

        self.analysesTribord = {}
        self.analysesTribord["derive"] = AnalyseDerive(878, 423, 895, 484)
        self.analysesTribord["safran"] = AnalyseSafran(520, 406, 609, 549, 562, 413, 578, 467)
        self.analysesTribord["mousse"] = AnalyseMousse(632, 415, 680, 526)

        self.analysesBabord = {}
        self.analysesBabord["derive"] = AnalyseDerive(850, 568, 888, 654)
        self.analysesBabord["safran"] = AnalyseSafran(1234, 537, 1337, 669, 1315, 550, 1262, 645)
        self.analysesBabord["mousse"] = AnalyseMousse(1170, 555, 1218, 665)
    ...
    def execute(self):
        ...
        for key, dataRecovery in self.listData.items():
            ...
            for aMeasureKey in dataRecovery.data:
                ...
                dataRecovery.data[aMeasureKey] = dataRecovery.data[aMeasureKey].loc[dataRecovery.data[aMeasureKey]["quality"] > self.analyses[aMeasureKey].qualityLimit+10]

                # Moyenne glissante
                dataRecovery.data[aMeasureKey]["height"] = uniform_filter1d(dataRecovery.data[aMeasureKey]["height"].values.tolist(), size=50)
